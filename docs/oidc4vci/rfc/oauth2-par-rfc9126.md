...
## [**Abstract**](https://www.rfc-editor.org/rfc/rfc9126.html#abstract)

This document defines the pushed authorization request (PAR) endpoint, which allows clients to push the payload of an OAuth 2.0 authorization request to the authorization server via a direct request and provides them with a request URI that is used as reference to the data in a subsequent call to the authorization endpoint.

## [**Status of This Memo**](https://www.rfc-editor.org/rfc/rfc9126.html#name-status-of-this-memo)

This is an Internet Standards Track document.

This document is a product of the Internet Engineering Task Force (IETF). It represents the consensus of the IETF community. It has received public review and has been approved for publication by the Internet Engineering Steering Group (IESG). Further information on Internet Standards is available in Section 2 of RFC 7841\.

Information about the current status of this document, any errata, and how to provide feedback on it may be obtained at [https://www.rfc-editor.org/info/rfc9126](https://www.rfc-editor.org/info/rfc9126).

## [**Copyright Notice**](https://www.rfc-editor.org/rfc/rfc9126.html#name-copyright-notice)
...
## [**1\.**](https://www.rfc-editor.org/rfc/rfc9126.html#section-1) **[Introduction](https://www.rfc-editor.org/rfc/rfc9126.html#name-introduction)**

This document defines the pushed authorization request (PAR) endpoint, which enables an OAuth \[[RFC6749](https://www.rfc-editor.org/rfc/rfc9126.html#RFC6749)\] client to push the payload of an authorization request directly to the authorization server. A request URI value is received in exchange; it is used as reference to the authorization request payload data in a subsequent call to the authorization endpoint via the user agent.

In OAuth \[[RFC6749](https://www.rfc-editor.org/rfc/rfc9126.html#RFC6749)\], authorization request parameters are typically sent as URI query parameters via redirection in the user agent. This is simple but also yields challenges:
...
JWT-Secured Authorization Request (JAR) \[[RFC9101](https://www.rfc-editor.org/rfc/rfc9126.html#RFC9101)\] provides solutions for the security challenges by allowing OAuth clients to wrap authorization request parameters in a Request Object, which is a signed and optionally encrypted JSON Web Token (JWT) \[[RFC7519](https://www.rfc-editor.org/rfc/rfc9126.html#RFC7519)\]. In order to cope with the size restrictions, JAR introduces the `request_uri` parameter that allows clients to send a reference to a Request Object instead of the Request Object itself.
...
Note that HTTP `POST` requests to the authorization endpoint via the user agent, as described in [Section 3.1](https://www.rfc-editor.org/rfc/rfc6749#section-3.1) of \[[RFC6749](https://www.rfc-editor.org/rfc/rfc9126.html#RFC6749)\] and Section 3.1.2.1 of \[[OIDC](https://www.rfc-editor.org/rfc/rfc9126.html#OIDC)\], could also be used to cope with the request size limitations described above. However, it's only optional per \[[RFC6749](https://www.rfc-editor.org/rfc/rfc9126.html#RFC6749)\], and, even when supported, it is a viable option for conventional web applications but is prohibitively difficult to use with installed mobile applications. As described in \[[RFC8252](https://www.rfc-editor.org/rfc/rfc9126.html#RFC8252)\], those apps use platform-specific APIs to open the authorization request URI in the system browser. When a mobile app launches a browser, however, the resultant initial request is constrained to use the `GET` method. Using `POST` for the authorization request would require the app to first direct the browser to open a URI that the app controls via `GET` while somehow conveying the sizable authorization request payload and then having the resultant response contain the content and script to initiate a cross-site form `POST` towards the authorization server. PAR is simpler to use and has additional security benefits, as described above.

### [**1.1.**](https://www.rfc-editor.org/rfc/rfc9126.html#section-1.1) **[Introductory Example](https://www.rfc-editor.org/rfc/rfc9126.html#name-introductory-example)**
...
### [**1.2.**](https://www.rfc-editor.org/rfc/rfc9126.html#section-1.2) **[Conventions and Terminology](https://www.rfc-editor.org/rfc/rfc9126.html#name-conventions-and-terminology)**

The key words "**MUST**", "**MUST NOT**", "**REQUIRED**", "**SHALL**", "**SHALL NOT**", "**SHOULD**", "**SHOULD NOT**", "**RECOMMENDED**", "**NOT RECOMMENDED**", "**MAY**", and "**OPTIONAL**" in this document are to be interpreted as described in BCP 14 \[[RFC2119](https://www.rfc-editor.org/rfc/rfc9126.html#RFC2119)\] \[[RFC8174](https://www.rfc-editor.org/rfc/rfc9126.html#RFC8174)\] when, and only when, they appear in all capitals, as shown here.

This specification uses the terms "access token", "authorization server", "authorization endpoint", "authorization request", "token endpoint", and "client" defined by "The OAuth 2.0 Authorization Framework" \[[RFC6749](https://www.rfc-editor.org/rfc/rfc9126.html#RFC6749)\].

## [**2\.**](https://www.rfc-editor.org/rfc/rfc9126.html#section-2) **[Pushed Authorization Request Endpoint](https://www.rfc-editor.org/rfc/rfc9126.html#name-pushed-authorization-reques)**

The pushed authorization request endpoint is an HTTP API at the authorization server that accepts HTTP `POST` requests with parameters in the HTTP request message body using the `application/x-www-form-urlencoded` format. This format has a character encoding of UTF-8, as described in [Appendix B](https://www.rfc-editor.org/rfc/rfc6749#appendix-B) of \[[RFC6749](https://www.rfc-editor.org/rfc/rfc9126.html#RFC6749)\]. The PAR endpoint URL **MUST** use the "https" scheme.

Authorization servers supporting PAR **SHOULD** include the URL of their pushed authorization request endpoint in their authorization server metadata document \[[RFC8414](https://www.rfc-editor.org/rfc/rfc9126.html#RFC8414)\] using the `pushed_authorization_request_endpoint` parameter as defined in [Section 5](https://www.rfc-editor.org/rfc/rfc9126.html#as_metadata).

The endpoint accepts the authorization request parameters defined in \[[RFC6749](https://www.rfc-editor.org/rfc/rfc9126.html#RFC6749)\] for the authorization endpoint as well as all applicable extensions defined for the authorization endpoint. Some examples of such extensions include Proof Key for Code Exchange (PKCE) \[[RFC7636](https://www.rfc-editor.org/rfc/rfc9126.html#RFC7636)\], Resource Indicators \[[RFC8707](https://www.rfc-editor.org/rfc/rfc9126.html#RFC8707)\], and OpenID Connect (OIDC) \[[OIDC](https://www.rfc-editor.org/rfc/rfc9126.html#OIDC)\]. The endpoint **MAY** also support sending the set of authorization request parameters as a Request Object according to \[[RFC9101](https://www.rfc-editor.org/rfc/rfc9126.html#RFC9101)\] and [Section 3](https://www.rfc-editor.org/rfc/rfc9126.html#request_parameter) of this document.

The rules for client authentication as defined in \[[RFC6749](https://www.rfc-editor.org/rfc/rfc9126.html#RFC6749)\] for token endpoint requests, including the applicable authentication methods, apply for the PAR endpoint as well. If applicable, the `token_endpoint_auth_method` client metadata parameter \[[RFC7591](https://www.rfc-editor.org/rfc/rfc9126.html#RFC7591)\] indicates the registered authentication method for the client to use when making direct requests to the authorization server, including requests to the PAR endpoint. Similarly, the `token_endpoint_auth_methods_supported` authorization server metadata \[[RFC8414](https://www.rfc-editor.org/rfc/rfc9126.html#RFC8414)\] parameter lists client authentication methods supported by the authorization server when accepting direct requests from clients, including requests to the PAR endpoint.
...
### [**2.1.**](https://www.rfc-editor.org/rfc/rfc9126.html#section-2.1) **[Request](https://www.rfc-editor.org/rfc/rfc9126.html#name-request)**
...
The client constructs the message body of an HTTP `POST` request with parameters formatted with `x-www-form-urlencoded` using a character encoding of UTF-8, as described in [Appendix B](https://www.rfc-editor.org/rfc/rfc6749#appendix-B) of \[[RFC6749](https://www.rfc-editor.org/rfc/rfc9126.html#RFC6749)\]. If applicable, the client also adds its authentication credentials to the request header or the request body using the same rules as for token endpoint requests.
...
1. Authenticate the client in the same way as at the token endpoint ([Section 2.3](https://www.rfc-editor.org/rfc/rfc6749#section-2.3) of \[[RFC6749](https://www.rfc-editor.org/rfc/rfc9126.html#RFC6749)\]).  
...
The authorization server **MAY** allow clients with authentication credentials to establish per-authorization-request redirect URIs with every pushed authorization request. Described in more detail in [Section 2.4](https://www.rfc-editor.org/rfc/rfc9126.html#redirect_uri_mgmt), this is possible since, in contrast to \[[RFC6749](https://www.rfc-editor.org/rfc/rfc9126.html#RFC6749)\], this specification gives the authorization server the ability to authenticate clients and validate client requests before the actual authorization request is performed.

### [**2.2.**](https://www.rfc-editor.org/rfc/rfc9126.html#section-2.2) **[Successful Response](https://www.rfc-editor.org/rfc/rfc9126.html#name-successful-response)**

If the verification is successful, the server **MUST** generate a request URI and provide it in the response with a `201` HTTP status code. The following parameters are included as top-level members in the message body of the HTTP response using the `application/json` media type as defined by \[[RFC8259](https://www.rfc-editor.org/rfc/rfc9126.html#RFC8259)\].
...
### [**2.3.**](https://www.rfc-editor.org/rfc/rfc9126.html#section-2.3) **[Error Response](https://www.rfc-editor.org/rfc/rfc9126.html#name-error-response)**

The authorization server returns an error response with the same format as is specified for error responses from the token endpoint in [Section 5.2](https://www.rfc-editor.org/rfc/rfc6749#section-5.2) of \[[RFC6749](https://www.rfc-editor.org/rfc/rfc9126.html#RFC6749)\] using the appropriate error code from therein or from [Section 4.1.2.1](https://www.rfc-editor.org/rfc/rfc6749#section-4.1.2.1) of \[[RFC6749](https://www.rfc-editor.org/rfc/rfc9126.html#RFC6749)\]. In those cases where [Section 4.1.2.1](https://www.rfc-editor.org/rfc/rfc6749#section-4.1.2.1) of \[[RFC6749](https://www.rfc-editor.org/rfc/rfc9126.html#RFC6749)\] prohibits automatic redirection with an error back to the requesting client and hence doesn't define an error code (for example, when the request fails due to a missing, invalid, or mismatching redirection URI), the `invalid_request` error code can be used as the default error code. Error codes defined by the OAuth extension can also be used when such an extension is involved in the initial processing of the authorization request that was pushed. Since initial processing of the pushed authorization request does not involve resource owner interaction, error codes related to user interaction, such as `consent_required` defined by \[[OIDC](https://www.rfc-editor.org/rfc/rfc9126.html#OIDC)\], are never returned.

If the client is required to use signed Request Objects, by either the authorization server or the client policy (see \[[RFC9101](https://www.rfc-editor.org/rfc/rfc9126.html#RFC9101)\], [Section 10.5](https://www.rfc-editor.org/rfc/rfc9101#section-10.5)), the authorization server **MUST** only accept requests complying with the definition given in [Section 3](https://www.rfc-editor.org/rfc/rfc9126.html#request_parameter) and **MUST** refuse any other request with HTTP status code 400 and error code `invalid_request`.
...
## [**3\.**](https://www.rfc-editor.org/rfc/rfc9126.html#section-3) **[The "request" Request Parameter](https://www.rfc-editor.org/rfc/rfc9126.html#name-the-request-request-paramet)**

Clients **MAY** use the `request` parameter as defined in JAR \[[RFC9101](https://www.rfc-editor.org/rfc/rfc9126.html#RFC9101)\] to push a Request Object JWT to the authorization server. The rules for processing, signing, and encryption of the Request Object as defined in JAR \[[RFC9101](https://www.rfc-editor.org/rfc/rfc9126.html#RFC9101)\] apply. Request parameters required by a given client authentication method are included in the `application/x-www-form-urlencoded` request directly and are the only parameters other than `request` in the form body (e.g., mutual TLS client authentication \[[RFC8705](https://www.rfc-editor.org/rfc/rfc9126.html#RFC8705)\] uses the `client_id` HTTP request parameter, while JWT assertion-based client authentication \[[RFC7523](https://www.rfc-editor.org/rfc/rfc9126.html#RFC7523)\] uses `client_assertion` and `client_assertion_type`). All other request parameters, i.e., those pertaining to the authorization request itself, **MUST** appear as claims of the JWT representing the authorization request.

The following is an example of a pushed authorization request using a signed Request Object with the same authorization request payload as the example in [Section 2.1](https://www.rfc-editor.org/rfc/rfc9126.html#request). The client is authenticated with JWT client assertion-based authentication \[[RFC7523](https://www.rfc-editor.org/rfc/rfc9126.html#RFC7523)\] (extra line breaks and spaces for display purposes only):
...
The authorization server **MUST** take the following steps beyond the processing rules defined in [Section 2.1](https://www.rfc-editor.org/rfc/rfc9126.html#request):

1. If applicable, decrypt the Request Object as specified in JAR \[[RFC9101](https://www.rfc-editor.org/rfc/rfc9126.html#RFC9101)\], [Section 6.1](https://www.rfc-editor.org/rfc/rfc9101#section-6.1).  
2. Validate the Request Object signature as specified in JAR \[[RFC9101](https://www.rfc-editor.org/rfc/rfc9126.html#RFC9101)\], [Section 6.2](https://www.rfc-editor.org/rfc/rfc9101#section-6.2).  
...
The following RSA key pair, represented in JSON Web Key (JWK) format \[[RFC7517](https://www.rfc-editor.org/rfc/rfc9126.html#RFC7517)\], can be used to validate or recreate the Request Object signature in the above example (extra line breaks and indentation within values for display purposes only):
...
## [**4\.**](https://www.rfc-editor.org/rfc/rfc9126.html#section-4) **[Authorization Request](https://www.rfc-editor.org/rfc/rfc9126.html#name-authorization-request)**

The client uses the `request_uri` value returned by the authorization server to build an authorization request as defined in \[[RFC9101](https://www.rfc-editor.org/rfc/rfc9126.html#RFC9101)\]. This is shown in the following example where the client directs the user agent to make the following HTTP request (extra line breaks and indentation for display purposes only):
...
Note: Authorization server and clients **MAY** use metadata as defined in Sections [5](https://www.rfc-editor.org/rfc/rfc9126.html#as_metadata) and [6](https://www.rfc-editor.org/rfc/rfc9126.html#c_metadata) to signal the desired behavior.

## [**5\.**](https://www.rfc-editor.org/rfc/rfc9126.html#section-5) **[Authorization Server Metadata](https://www.rfc-editor.org/rfc/rfc9126.html#name-authorization-server-metada)**

The following authorization server metadata parameters \[[RFC8414](https://www.rfc-editor.org/rfc/rfc9126.html#RFC8414)\] are introduced to signal the server's capability and policy with respect to PAR.
...
## [**7\.**](https://www.rfc-editor.org/rfc/rfc9126.html#section-7) **[Security Considerations](https://www.rfc-editor.org/rfc/rfc9126.html#name-security-considerations)**

### [**7.1.**](https://www.rfc-editor.org/rfc/rfc9126.html#section-7.1) **[Request URI Guessing](https://www.rfc-editor.org/rfc/rfc9126.html#name-request-uri-guessing)**

An attacker could attempt to guess and replay a valid request URI value and try to impersonate the respective client. The authorization server **MUST** account for the considerations given in JAR \[[RFC9101](https://www.rfc-editor.org/rfc/rfc9126.html#RFC9101)\], [Section 10.2](https://www.rfc-editor.org/rfc/rfc9101#section-10.2), clause (d) on request URI entropy.

### [**7.2.**](https://www.rfc-editor.org/rfc/rfc9126.html#section-7.2) **[Open Redirection](https://www.rfc-editor.org/rfc/rfc9126.html#name-open-redirection)**
...
### [**7.3.**](https://www.rfc-editor.org/rfc/rfc9126.html#section-7.3) **[Request Object Replay](https://www.rfc-editor.org/rfc/rfc9126.html#name-request-object-replay)**
...
### [**7.4.**](https://www.rfc-editor.org/rfc/rfc9126.html#section-7.4) **[Client Policy Change](https://www.rfc-editor.org/rfc/rfc9126.html#name-client-policy-change)**
...
### [**7.5.**](https://www.rfc-editor.org/rfc/rfc9126.html#section-7.5) **[Request URI Swapping](https://www.rfc-editor.org/rfc/rfc9126.html#name-request-uri-swapping)**

An attacker could capture the request URI from one request and then substitute it into a different authorization request. For example, in the context of OpenID Connect, an attacker could replace a request URI asking for a high level of authentication assurance with one that requires a lower level of assurance. Clients **SHOULD** make use of PKCE \[[RFC7636](https://www.rfc-editor.org/rfc/rfc9126.html#RFC7636)\], a unique `state` parameter \[[RFC6749](https://www.rfc-editor.org/rfc/rfc9126.html#RFC6749)\], or the OIDC "nonce" parameter \[[OIDC](https://www.rfc-editor.org/rfc/rfc9126.html#OIDC)\] in the pushed Request Object to prevent this attack.

## [**8\.**](https://www.rfc-editor.org/rfc/rfc9126.html#section-8) **[Privacy Considerations](https://www.rfc-editor.org/rfc/rfc9126.html#name-privacy-considerations)**
...
## [**9\.**](https://www.rfc-editor.org/rfc/rfc9126.html#section-9) **[IANA Considerations](https://www.rfc-editor.org/rfc/rfc9126.html#name-iana-considerations)**

### [**9.1.**](https://www.rfc-editor.org/rfc/rfc9126.html#section-9.1) **[OAuth Authorization Server Metadata](https://www.rfc-editor.org/rfc/rfc9126.html#name-oauth-authorization-server-)**

IANA has registered the following values in the IANA "OAuth Authorization Server Metadata" registry of \[[IANA.OAuth.Parameters](https://www.rfc-editor.org/rfc/rfc9126.html#IANA.OAuth.Parameters)\] established by \[[RFC8414](https://www.rfc-editor.org/rfc/rfc9126.html#RFC8414)\].
...
[Section 5](https://www.rfc-editor.org/rfc/rfc9126.html#as_metadata) of RFC 9126  
Metadata Name:  
...
### [**9.2.**](https://www.rfc-editor.org/rfc/rfc9126.html#section-9.2) **[OAuth Dynamic Client Registration Metadata](https://www.rfc-editor.org/rfc/rfc9126.html#name-oauth-dynamic-client-regist)**

IANA has registered the following value in the IANA "OAuth Dynamic Client Registration Metadata" registry of \[[IANA.OAuth.Parameters](https://www.rfc-editor.org/rfc/rfc9126.html#IANA.OAuth.Parameters)\] established by \[[RFC7591](https://www.rfc-editor.org/rfc/rfc9126.html#RFC7591)\].
...
IESG  
Specification Document(s):  
[Section 6](https://www.rfc-editor.org/rfc/rfc9126.html#c_metadata) of RFC 9126

### [**9.3.**](https://www.rfc-editor.org/rfc/rfc9126.html#section-9.3) **[OAuth URI Registration](https://www.rfc-editor.org/rfc/rfc9126.html#name-oauth-uri-registration)**

IANA has registered the following value in the "OAuth URI" registry of \[[IANA.OAuth.Parameters](https://www.rfc-editor.org/rfc/rfc9126.html#IANA.OAuth.Parameters)\] established by \[[RFC6755](https://www.rfc-editor.org/rfc/rfc9126.html#RFC6755)\].
...
IESG  
Specification Document(s):  
[Section 2.2](https://www.rfc-editor.org/rfc/rfc9126.html#par-response) of RFC 9126

## [**10\.**](https://www.rfc-editor.org/rfc/rfc9126.html#section-10) **[References](https://www.rfc-editor.org/rfc/rfc9126.html#name-references)**

### [**10.1.**](https://www.rfc-editor.org/rfc/rfc9126.html#section-10.1) **[Normative References](https://www.rfc-editor.org/rfc/rfc9126.html#name-normative-references)**
...
### [**10.2.**](https://www.rfc-editor.org/rfc/rfc9126.html#section-10.2) **[Informative References](https://www.rfc-editor.org/rfc/rfc9126.html#name-informative-references)**

**\[IANA.OAuth.Parameters\]**

IANA, "OAuth Parameters", \<[http://www.iana.org/assignments/oauth-parameters](http://www.iana.org/assignments/oauth-parameters)\>.
...
Hardt, D., Parecki, A., and T. Lodderstedt, "The OAuth 2.1 Authorization Framework", Work in Progress, Internet-Draft, draft-ietf-oauth-v2-1-03, 8 September 2021, \<[https://datatracker.ietf.org/doc/html/draft-ietf-oauth-v2-1-03](https://datatracker.ietf.org/doc/html/draft-ietf-oauth-v2-1-03)\>.
...
## [**Acknowledgements**](https://www.rfc-editor.org/rfc/rfc9126.html#name-acknowledgements)

This specification is based on the work on [Pushed Request Object](https://bitbucket.org/openid/fapi/src/master/Financial_API_Pushed_Request_Object.md) conducted at the Financial-grade API Working Group at the OpenID Foundation. We would like to thank the members of the WG for their valuable contributions.
...
## [**Authors' Addresses**](https://www.rfc-editor.org/rfc/rfc9126.html#name-authors-addresses)
