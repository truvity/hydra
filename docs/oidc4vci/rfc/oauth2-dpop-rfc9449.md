...
## [**Abstract**](https://www.rfc-editor.org/rfc/rfc9449.html#abstract)

This document describes a mechanism for sender-constraining OAuth 2.0 tokens via a proof-of-possession mechanism on the application level. This mechanism allows for the detection of replay attacks with access and refresh tokens.

## [**Status of This Memo**](https://www.rfc-editor.org/rfc/rfc9449.html#name-status-of-this-memo)

This is an Internet Standards Track document.

This document is a product of the Internet Engineering Task Force (IETF). It represents the consensus of the IETF community. It has received public review and has been approved for publication by the Internet Engineering Steering Group (IESG). Further information on Internet Standards is available in Section 2 of RFC 7841\.

Information about the current status of this document, any errata, and how to provide feedback on it may be obtained at [https://www.rfc-editor.org/info/rfc9449](https://www.rfc-editor.org/info/rfc9449).

## [**Copyright Notice**](https://www.rfc-editor.org/rfc/rfc9449.html#name-copyright-notice)
...
### [**1.1.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-1.1) **[Conventions and Terminology](https://www.rfc-editor.org/rfc/rfc9449.html#name-conventions-and-terminology)**

The key words "**MUST**", "**MUST NOT**", "**REQUIRED**", "**SHALL**", "**SHALL NOT**", "**SHOULD**", "**SHOULD NOT**", "**RECOMMENDED**", "**NOT RECOMMENDED**", "**MAY**", and "**OPTIONAL**" in this document are to be interpreted as described in BCP 14 \[[RFC2119](https://www.rfc-editor.org/rfc/rfc9449.html#RFC2119)\] \[[RFC8174](https://www.rfc-editor.org/rfc/rfc9449.html#RFC8174)\] when, and only when, they appear in all capitals, as shown here.

This specification uses the Augmented Backus-Naur Form (ABNF) notation of \[[RFC5234](https://www.rfc-editor.org/rfc/rfc9449.html#RFC5234)\].

This specification uses the terms "access token", "refresh token", "authorization server", "resource server", "authorization endpoint", "authorization request", "authorization response", "token endpoint", "grant type", "access token request", "access token response", "client", "public client", and "confidential client" defined by "The OAuth 2.0 Authorization Framework" \[[RFC6749](https://www.rfc-editor.org/rfc/rfc9449.html#RFC6749)\].

The terms "request", "response", "header field", and "target URI" are imported from \[[RFC9110](https://www.rfc-editor.org/rfc/rfc9449.html#RFC9110)\].

The terms "JOSE" and "JOSE Header" are imported from \[[RFC7515](https://www.rfc-editor.org/rfc/rfc9449.html#RFC7515)\].

This document contains non-normative examples of partial and complete HTTP messages. Some examples use a single trailing backslash to indicate line wrapping for long values, as per \[[RFC8792](https://www.rfc-editor.org/rfc/rfc9449.html#RFC8792)\]. The character and leading spaces on wrapped lines are not part of the value.

## [**2\.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-2) **[Objectives](https://www.rfc-editor.org/rfc/rfc9449.html#name-objectives)**
...
Access tokens that are sender-constrained via DPoP thus stand in contrast to the typical bearer token, which can be used by any party in possession of such a token. Although protections generally exist to prevent unintended disclosure of bearer tokens, unforeseen vectors for leakage have occurred due to vulnerabilities and implementation issues in other layers in the protocol or software stack (see, e.g., Compression Ratio Info-leak Made Easy (CRIME) \[[CRIME](https://www.rfc-editor.org/rfc/rfc9449.html#CRIME)\], Browser Reconnaissance and Exfiltration via Adaptive Compression of Hypertext (BREACH) \[[BREACH](https://www.rfc-editor.org/rfc/rfc9449.html#BREACH)\], Heartbleed \[[Heartbleed](https://www.rfc-editor.org/rfc/rfc9449.html#Heartbleed)\], and the Cloudflare parser bug \[[Cloudbleed](https://www.rfc-editor.org/rfc/rfc9449.html#Cloudbleed)\]). There have also been numerous published token theft attacks on OAuth implementations themselves (\[[GitHub.Tokens](https://www.rfc-editor.org/rfc/rfc9449.html#GitHub.Tokens)\] is just one high-profile example). DPoP provides a general defense in depth against the impact of unanticipated token leakage. DPoP is not, however, a substitute for a secure transport and **MUST** always be used in conjunction with HTTPS.

The very nature of the typical OAuth protocol interaction necessitates that the client discloses the access token to the protected resources that it accesses. The attacker model in \[[SECURITY-TOPICS](https://www.rfc-editor.org/rfc/rfc9449.html#I-D.ietf-oauth-security-topics)\] describes cases where a protected resource might be counterfeit, malicious, or compromised and plays received tokens against other protected resources to gain unauthorized access. Audience-restricted access tokens (e.g., using the JWT \[[RFC7519](https://www.rfc-editor.org/rfc/rfc9449.html#RFC7519)\] `aud` claim) can prevent such misuse. However, doing so in practice has proven to be prohibitively cumbersome for many deployments (despite extensions such as \[[RFC8707](https://www.rfc-editor.org/rfc/rfc9449.html#RFC8707)\]). Sender-constraining access tokens is a more robust and straightforward mechanism to prevent such token replay at a different endpoint, and DPoP is an accessible application-layer means of doing so.

Due to the potential for cross-site scripting (XSS), browser-based OAuth clients bring to bear added considerations with respect to protecting tokens. The most straightforward XSS-based attack is for an attacker to exfiltrate a token and use it themselves completely independent of the legitimate client. A stolen access token is used for protected resource access, and a stolen refresh token is used for obtaining new access tokens. If the private key is non-extractable (as is possible with \[[W3C.WebCryptoAPI](https://www.rfc-editor.org/rfc/rfc9449.html#W3C.WebCryptoAPI)\]), DPoP renders exfiltrated tokens alone unusable.
...
Additional security considerations are discussed in [Section 11](https://www.rfc-editor.org/rfc/rfc9449.html#Security).

## [**3\.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-3) **[Concept](https://www.rfc-editor.org/rfc/rfc9449.html#name-concept)**
...
[*Figure 1*](https://www.rfc-editor.org/rfc/rfc9449.html#figure-1)*: [Basic DPoP Flow](https://www.rfc-editor.org/rfc/rfc9449.html#name-basic-dpop-flow)*  
The basic steps of an OAuth flow with DPoP (without the optional nonce) are shown in [Figure 1](https://www.rfc-editor.org/rfc/rfc9449.html#basic-flow).

1. In the token request, the client sends an authorization grant (e.g., an authorization code, refresh token, etc.) to the authorization server in order to obtain an access token (and potentially a refresh token). The client attaches a DPoP proof to the request in an HTTP header.  
...
4. The resource server refuses to serve the request if the signature check fails or if the data in the DPoP proof is wrong, e.g., the target URI does not match the URI claim in the DPoP proof JWT. The access token itself, of course, must also be valid in all other respects.
...
DPoP does not directly ensure message integrity, but it relies on the TLS layer for that purpose. See [Section 11](https://www.rfc-editor.org/rfc/rfc9449.html#Security) for details.

## [**4\.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-4) **[DPoP Proof JWTs](https://www.rfc-editor.org/rfc/rfc9449.html#name-dpop-proof-jwts)**
...
A valid DPoP proof demonstrates to the server that the client holds the private key that was used to sign the DPoP proof JWT. This enables authorization servers to bind issued tokens to the corresponding public key (as described in [Section 5](https://www.rfc-editor.org/rfc/rfc9449.html#access-token-request)) and enables resource servers to verify the key-binding of tokens that it receives (see [Section 7.1](https://www.rfc-editor.org/rfc/rfc9449.html#http-auth-scheme)), which prevents said tokens from being used by any entity that does not have access to the private key.

The DPoP proof demonstrates possession of a key and, by itself, is not an authentication or access control mechanism. When presented in conjunction with a key-bound access token as described in [Section 7.1](https://www.rfc-editor.org/rfc/rfc9449.html#http-auth-scheme), the DPoP proof provides additional assurance about the legitimacy of the client to present the access token. However, a valid DPoP proof JWT is not sufficient alone to make access control decisions.

### [**4.1.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-4.1) **[The DPoP HTTP Header](https://www.rfc-editor.org/rfc/rfc9449.html#name-the-dpop-http-header)**

A DPoP proof is included in an HTTP request using the following request header field.

`DPoP`:  
A JWT that adheres to the structure and syntax of [Section 4.2](https://www.rfc-editor.org/rfc/rfc9449.html#DPoP-Proof-Syntax).  
[Figure 2](https://www.rfc-editor.org/rfc/rfc9449.html#dpop-proof-jwt) shows an example DPoP HTTP header field. The example uses "\\" line wrapping per \[[RFC8792](https://www.rfc-editor.org/rfc/rfc9449.html#RFC8792)\].
...
[*Figure 2*](https://www.rfc-editor.org/rfc/rfc9449.html#figure-2)*: [Example `DPoP` Header](https://www.rfc-editor.org/rfc/rfc9449.html#name-example-dpop-header)*  
Note that per \[[RFC9110](https://www.rfc-editor.org/rfc/rfc9449.html#RFC9110)\], header field names are case insensitive; thus, `DPoP`, `DPOP`, `dpop`, etc., are all valid and equivalent header field names. However, case is significant in the header field value.

The `DPoP` HTTP header field value uses the token68 syntax defined in [Section 11.2](https://rfc-editor.org/rfc/rfc9110#section-11.2) of \[[RFC9110](https://www.rfc-editor.org/rfc/rfc9449.html#RFC9110)\] and is repeated below in [Figure 3](https://www.rfc-editor.org/rfc/rfc9449.html#dpop-header-abnf) for ease of reference.
...
[*Figure 3*](https://www.rfc-editor.org/rfc/rfc9449.html#figure-3)*: [DPoP Header Field ABNF](https://www.rfc-editor.org/rfc/rfc9449.html#name-dpop-header-field-abnf)*

### [**4.2.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-4.2) **[DPoP Proof JWT Syntax](https://www.rfc-editor.org/rfc/rfc9449.html#name-dpop-proof-jwt-syntax)**

A DPoP proof is a JWT \[[RFC7519](https://www.rfc-editor.org/rfc/rfc9449.html#RFC7519)\] that is signed (using JSON Web Signature (JWS) \[[RFC7515](https://www.rfc-editor.org/rfc/rfc9449.html#RFC7515)\]) with a private key chosen by the client (see below). The JOSE Header of a DPoP JWT **MUST** contain at least the following parameters:

`typ`:  
A field with the value `dpop+jwt`, which explicitly types the DPoP proof JWT as recommended in [Section 3.11](https://rfc-editor.org/rfc/rfc8725#section-3.11) of \[[RFC8725](https://www.rfc-editor.org/rfc/rfc9449.html#RFC8725)\].

`alg`:  
An identifier for a JWS asymmetric digital signature algorithm from \[[IANA.JOSE.ALGS](https://www.rfc-editor.org/rfc/rfc9449.html#IANA.JOSE.ALGS)\]. It **MUST NOT** be `none` or an identifier for a symmetric algorithm (Message Authentication Code (MAC)).

`jwk`:  
Represents the public key chosen by the client in JSON Web Key (JWK) \[[RFC7517](https://www.rfc-editor.org/rfc/rfc9449.html#RFC7517)\] format as defined in [Section 4.1.3](https://rfc-editor.org/rfc/rfc7515#section-4.1.3) of \[[RFC7515](https://www.rfc-editor.org/rfc/rfc9449.html#RFC7515)\]. It **MUST NOT** contain a private key.

The payload of a DPoP proof **MUST** contain at least the following claims:

`jti`:  
Unique identifier for the DPoP proof JWT. The value **MUST** be assigned such that there is a negligible probability that the same value will be assigned to any other DPoP proof used in the same context during the time window of validity. Such uniqueness can be accomplished by encoding (base64url or any other suitable encoding) at least 96 bits of pseudorandom data or by using a version 4 Universally Unique Identifier (UUID) string according to \[[RFC4122](https://www.rfc-editor.org/rfc/rfc9449.html#RFC4122)\]. The `jti` can be used by the server for replay detection and prevention; see [Section 11.1](https://www.rfc-editor.org/rfc/rfc9449.html#Token_Replay).

`htm`:  
The value of the HTTP method ([Section 9.1](https://rfc-editor.org/rfc/rfc9110#section-9.1) of \[[RFC9110](https://www.rfc-editor.org/rfc/rfc9449.html#RFC9110)\]) of the request to which the JWT is attached.

`htu`:  
The HTTP target URI ([Section 7.1](https://rfc-editor.org/rfc/rfc9110#section-7.1) of \[[RFC9110](https://www.rfc-editor.org/rfc/rfc9449.html#RFC9110)\]) of the request to which the JWT is attached, without query and fragment parts.

`iat`:  
Creation timestamp of the JWT ([Section 4.1.6](https://rfc-editor.org/rfc/rfc7519#section-4.1.6) of \[[RFC7519](https://www.rfc-editor.org/rfc/rfc9449.html#RFC7519)\]).

When the DPoP proof is used in conjunction with the presentation of an access token in protected resource access (see [Section 7](https://www.rfc-editor.org/rfc/rfc9449.html#protected-resource-access)), the DPoP proof **MUST** also contain the following claim:
...
When the authentication server or resource server provides a `DPoP-Nonce` HTTP header in a response (see Sections [8](https://www.rfc-editor.org/rfc/rfc9449.html#ASNonce) and [9](https://www.rfc-editor.org/rfc/rfc9449.html#RSNonce)), the DPoP proof **MUST** also contain the following claim:
...
[Figure 4](https://www.rfc-editor.org/rfc/rfc9449.html#dpop-proof) is a conceptual example showing the decoded content of the DPoP proof in [Figure 2](https://www.rfc-editor.org/rfc/rfc9449.html#dpop-proof-jwt). The JSON of the JWT header and payload are shown, but the signature part is omitted. As usual, line breaks and extra spaces are included for formatting and readability.
...
[*Figure 4*](https://www.rfc-editor.org/rfc/rfc9449.html#figure-4)*: [Example JWT Content of a `DPoP` Proof](https://www.rfc-editor.org/rfc/rfc9449.html#name-example-jwt-content-of-a-dp)*  
Of the HTTP request, only the HTTP method and URI are included in the DPoP JWT; therefore, only these two message parts are covered by the DPoP proof. The idea is to sign just enough of the HTTP data to provide reasonable proof of possession with respect to the HTTP request. This design approach of using only a minimal subset of the HTTP header data is to avoid the substantial difficulties inherent in attempting to normalize HTTP messages. Nonetheless, DPoP proofs can be extended to contain other information of the HTTP request (see also [Section 11.7](https://www.rfc-editor.org/rfc/rfc9449.html#request_integrity)).

### [**4.3.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-4.3) **[Checking DPoP Proofs](https://www.rfc-editor.org/rfc/rfc9449.html#name-checking-dpop-proofs)**
...
3. All required claims per [Section 4.2](https://www.rfc-editor.org/rfc/rfc9449.html#DPoP-Proof-Syntax) are contained in the JWT.  
4. The `typ` JOSE Header Parameter has the value `dpop+jwt`.  
5. The `alg` JOSE Header Parameter indicates a registered asymmetric digital signature algorithm \[[IANA.JOSE.ALGS](https://www.rfc-editor.org/rfc/rfc9449.html#IANA.JOSE.ALGS)\], is not `none`, is supported by the application, and is acceptable per local policy.  
...
11. The creation time of the JWT, as determined by either the `iat` claim or a server managed timestamp via the `nonce` claim, is within an acceptable window (see [Section 11.1](https://www.rfc-editor.org/rfc/rfc9449.html#Token_Replay)).  
...
To reduce the likelihood of false negatives, servers **SHOULD** employ syntax-based normalization ([Section 6.2.2](https://rfc-editor.org/rfc/rfc3986#section-6.2.2) of \[[RFC3986](https://www.rfc-editor.org/rfc/rfc9449.html#RFC3986)\]) and scheme-based normalization ([Section 6.2.3](https://rfc-editor.org/rfc/rfc3986#section-6.2.3) of \[[RFC3986](https://www.rfc-editor.org/rfc/rfc9449.html#RFC3986)\]) before comparing the `htu` claim.

These checks may be performed in any order.

## [**5\.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-5) **[DPoP Access Token Request](https://www.rfc-editor.org/rfc/rfc9449.html#name-dpop-access-token-request)**

To request an access token that is bound to a public key using DPoP, the client **MUST** provide a valid DPoP proof JWT in a `DPoP` header when making an access token request to the authorization server's token endpoint. This is applicable for all access token requests regardless of grant type (e.g., the common `authorization_code` and `refresh_token` grant types and extension grants such as the JWT authorization grant \[[RFC7523](https://www.rfc-editor.org/rfc/rfc9449.html#RFC7523)\]). The HTTP request shown in [Figure 5](https://www.rfc-editor.org/rfc/rfc9449.html#token-request-code) illustrates such an access token request using an authorization code grant with a DPoP proof JWT in the `DPoP` header. [Figure 5](https://www.rfc-editor.org/rfc/rfc9449.html#token-request-code) uses "\\" line wrapping per \[[RFC8792](https://www.rfc-editor.org/rfc/rfc9449.html#RFC8792)\].
...
[*Figure 5*](https://www.rfc-editor.org/rfc/rfc9449.html#figure-5)*: [Token Request for a DPoP Sender-Constrained Token Using an Authorization Code](https://www.rfc-editor.org/rfc/rfc9449.html#name-token-request-for-a-dpop-se)*  
The `DPoP` HTTP header field **MUST** contain a valid DPoP proof JWT. If the DPoP proof is invalid, the authorization server issues an error response per [Section 5.2](https://rfc-editor.org/rfc/rfc6749#section-5.2) of \[[RFC6749](https://www.rfc-editor.org/rfc/rfc9449.html#RFC6749)\] with `invalid_dpop_proof` as the value of the `error` parameter.

To sender-constrain the access token after checking the validity of the DPoP proof, the authorization server associates the issued access token with the public key from the DPoP proof, which can be accomplished as described in [Section 6](https://www.rfc-editor.org/rfc/rfc9449.html#Confirmation). A `token_type` of `DPoP` **MUST** be included in the access token response to signal to the client that the access token was bound to its DPoP key and can be used as described in [Section 7.1](https://www.rfc-editor.org/rfc/rfc9449.html#http-auth-scheme). The example response shown in [Figure 6](https://www.rfc-editor.org/rfc/rfc9449.html#token-response) illustrates such a response.
...
[*Figure 6*](https://www.rfc-editor.org/rfc/rfc9449.html#figure-6)*: [Access Token Response](https://www.rfc-editor.org/rfc/rfc9449.html#name-access-token-response)*  
The example response in [Figure 6](https://www.rfc-editor.org/rfc/rfc9449.html#token-response) includes a refresh token that the client can use to obtain a new access token when the previous one expires. Refreshing an access token is a token request using the `refresh_token` grant type made to the authorization server's token endpoint. As with all access token requests, the client makes it a DPoP request by including a DPoP proof, as shown in [Figure 7](https://www.rfc-editor.org/rfc/rfc9449.html#token-request-rt). [Figure 7](https://www.rfc-editor.org/rfc/rfc9449.html#token-request-rt) uses "\\" line wrapping per \[[RFC8792](https://www.rfc-editor.org/rfc/rfc9449.html#RFC8792)\].
...
[*Figure 7*](https://www.rfc-editor.org/rfc/rfc9449.html#figure-7)*: [Token Request for a DPoP-Bound Token Using a Refresh Token](https://www.rfc-editor.org/rfc/rfc9449.html#name-token-request-for-a-dpop-bo)*  
...
An authorization server **MAY** elect to issue access tokens that are not DPoP bound, which is signaled to the client with a value of `Bearer` in the `token_type` parameter of the access token response per \[[RFC6750](https://www.rfc-editor.org/rfc/rfc9449.html#RFC6750)\]. For a public client that is also issued a refresh token, this has the effect of DPoP-binding the refresh token alone, which can improve the security posture even when protected resources are not updated to support DPoP.
...
Refresh tokens issued to confidential clients (those having established authentication credentials with the authorization server) are not bound to the DPoP proof public key because they are already sender-constrained with a different existing mechanism. The OAuth 2.0 Authorization Framework \[[RFC6749](https://www.rfc-editor.org/rfc/rfc9449.html#RFC6749)\] already requires that an authorization server bind refresh tokens to the client to which they were issued and that confidential clients authenticate to the authorization server when presenting a refresh token. As a result, such refresh tokens are sender-constrained by way of the client identifier and the associated authentication requirement. This existing sender-constraining mechanism is more flexible (e.g., it allows credential rotation for the client without invalidating refresh tokens) than binding directly to a particular public key.

### [**5.1.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-5.1) **[Authorization Server Metadata](https://www.rfc-editor.org/rfc/rfc9449.html#name-authorization-server-metada)**

This document introduces the following authorization server metadata \[[RFC8414](https://www.rfc-editor.org/rfc/rfc9449.html#RFC8414)\] parameter to signal support for DPoP in general and the specific JWS `alg` values the authorization server supports for DPoP proof JWTs.

`dpop_signing_alg_values_supported`:  
A JSON array containing a list of the JWS `alg` values (from the \[[IANA.JOSE.ALGS](https://www.rfc-editor.org/rfc/rfc9449.html#IANA.JOSE.ALGS)\] registry) supported by the authorization server for DPoP proof JWTs.

### [**5.2.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-5.2) **[Client Registration Metadata](https://www.rfc-editor.org/rfc/rfc9449.html#name-client-registration-metadat)**

The Dynamic Client Registration Protocol \[[RFC7591](https://www.rfc-editor.org/rfc/rfc9449.html#RFC7591)\] defines an API for dynamically registering OAuth 2.0 client metadata with authorization servers. The metadata defined by \[[RFC7591](https://www.rfc-editor.org/rfc/rfc9449.html#RFC7591)\], and registered extensions to it, also imply a general data model for clients that is useful for authorization server implementations even when the Dynamic Client Registration Protocol isn't in play. Such implementations will typically have some sort of user interface available for managing client configuration.

This document introduces the following client registration metadata \[[RFC7591](https://www.rfc-editor.org/rfc/rfc9449.html#RFC7591)\] parameter to indicate that the client always uses DPoP when requesting tokens from the authorization server.
...
## [**6\.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-6) **[Public Key Confirmation](https://www.rfc-editor.org/rfc/rfc9449.html#name-public-key-confirmation)**

Resource servers **MUST** be able to reliably identify whether an access token is DPoP-bound and ascertain sufficient information to verify the binding to the public key of the DPoP proof (see [Section 7.1](https://www.rfc-editor.org/rfc/rfc9449.html#http-auth-scheme)). Such a binding is accomplished by associating the public key with the token in a way that can be accessed by the protected resource, such as embedding the JWK hash in the issued access token directly, using the syntax described in [Section 6.1](https://www.rfc-editor.org/rfc/rfc9449.html#jwk-thumb-jwt), or through token introspection as described in [Section 6.2](https://www.rfc-editor.org/rfc/rfc9449.html#jwk-thumb-intro). Other methods of associating a public key with an access token are possible per an agreement by the authorization server and the protected resource; however, they are beyond the scope of this specification.
...
### [**6.1.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-6.1) **[JWK Thumbprint Confirmation Method](https://www.rfc-editor.org/rfc/rfc9449.html#name-jwk-thumbprint-confirmation)**

When access tokens are represented as JWTs \[[RFC7519](https://www.rfc-editor.org/rfc/rfc9449.html#RFC7519)\], the public key information is represented using the `jkt` confirmation method member defined herein. To convey the hash of a public key in a JWT, this specification introduces the following JWT Confirmation Method \[[RFC7800](https://www.rfc-editor.org/rfc/rfc9449.html#RFC7800)\] member for use under the `cnf` claim.

`jkt`:  
JWK SHA-256 Thumbprint confirmation method. The value of the `jkt` member **MUST** be the base64url encoding (as defined in \[[RFC7515](https://www.rfc-editor.org/rfc/rfc9449.html#RFC7515)\]) of the JWK SHA-256 Thumbprint (according to \[[RFC7638](https://www.rfc-editor.org/rfc/rfc9449.html#RFC7638)\]) of the DPoP public key (in JWK format) to which the access token is bound.  
The following example JWT in [Figure 8](https://www.rfc-editor.org/rfc/rfc9449.html#cnf-claim-jwt) with a decoded JWT payload shown in [Figure 9](https://www.rfc-editor.org/rfc/rfc9449.html#cnf-claim) contains a `cnf` claim with the `jkt` JWK Thumbprint confirmation method member. The `jkt` value in these examples is the hash of the public key from the DPoP proofs in the examples shown in [Section 5](https://www.rfc-editor.org/rfc/rfc9449.html#access-token-request). The example uses "\\" line wrapping per \[[RFC8792](https://www.rfc-editor.org/rfc/rfc9449.html#RFC8792)\].
...
[*Figure 8*](https://www.rfc-editor.org/rfc/rfc9449.html#figure-8)*: [JWT Containing a JWK SHA-256 Thumbprint Confirmation](https://www.rfc-editor.org/rfc/rfc9449.html#name-jwt-containing-a-jwk-sha-25)*
...
[*Figure 9*](https://www.rfc-editor.org/rfc/rfc9449.html#figure-9)*: [JWT Claims Set with a JWK SHA-256 Thumbprint Confirmation](https://www.rfc-editor.org/rfc/rfc9449.html#name-jwt-claims-set-with-a-jwk-s)*

### [**6.2.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-6.2) **[JWK Thumbprint Confirmation Method in Token Introspection](https://www.rfc-editor.org/rfc/rfc9449.html#name-jwk-thumbprint-confirmation-)**

"OAuth 2.0 Token Introspection" \[[RFC7662](https://www.rfc-editor.org/rfc/rfc9449.html#RFC7662)\] defines a method for a protected resource to query an authorization server about the active state of an access token. The protected resource also determines metainformation about the token.

For a DPoP-bound access token, the hash of the public key to which the token is bound is conveyed to the protected resource as metainformation in a token introspection response. The hash is conveyed using the same `cnf` content with `jkt` member structure as the JWK Thumbprint confirmation method, described in [Section 6.1](https://www.rfc-editor.org/rfc/rfc9449.html#jwk-thumb-jwt), as a top-level member of the introspection response JSON. Note that the resource server does not send a DPoP proof with the introspection request, and the authorization server does not validate an access token's DPoP binding at the introspection endpoint. Rather, the resource server uses the data of the introspection response to validate the access token binding itself locally.

If the `token_type` member is included in the introspection response, it **MUST** contain the value `DPoP`.

The example introspection request in [Figure 10](https://www.rfc-editor.org/rfc/rfc9449.html#introspect-req) and corresponding response in [Figure 11](https://www.rfc-editor.org/rfc/rfc9449.html#introspect-resp) illustrate an introspection exchange for the example DPoP-bound access token that was issued in [Figure 6](https://www.rfc-editor.org/rfc/rfc9449.html#token-response).
...
[*Figure 10*](https://www.rfc-editor.org/rfc/rfc9449.html#figure-10)*: [Example Introspection Request](https://www.rfc-editor.org/rfc/rfc9449.html#name-example-introspection-reque)*
...
[*Figure 11*](https://www.rfc-editor.org/rfc/rfc9449.html#figure-11)*: [Example Introspection Response for a DPoP-Bound Access Token](https://www.rfc-editor.org/rfc/rfc9449.html#name-example-introspection-respo)*

## [**7\.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-7) **[Protected Resource Access](https://www.rfc-editor.org/rfc/rfc9449.html#name-protected-resource-access)**

Requests to DPoP-protected resources **MUST** include both a DPoP proof as per [Section 4](https://www.rfc-editor.org/rfc/rfc9449.html#the-proof) and the access token as described in [Section 7.1](https://www.rfc-editor.org/rfc/rfc9449.html#http-auth-scheme). The DPoP proof **MUST** include the `ath` claim with a valid hash of the associated access token.
...
The resource server is required to calculate the hash of the token value presented and verify that it is the same as the hash value in the `ath` field as described in [Section 4.3](https://www.rfc-editor.org/rfc/rfc9449.html#checking). Since the `ath` field value is covered by the DPoP proof's signature, its inclusion binds the access token value to the holder of the key used to generate the signature.
...
### [**7.1.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-7.1) **[The DPoP Authentication Scheme](https://www.rfc-editor.org/rfc/rfc9449.html#name-the-dpop-authentication-sch)**

A DPoP-bound access token is sent using the `Authorization` request header field per [Section 11.6.2](https://rfc-editor.org/rfc/rfc9110#section-11.6.2) of \[[RFC9110](https://www.rfc-editor.org/rfc/rfc9449.html#RFC9110)\] with an authentication scheme of `DPoP`. The syntax of the `Authorization` header field for the `DPoP` scheme uses the token68 syntax defined in [Section 11.2](https://rfc-editor.org/rfc/rfc9110#section-11.2) of \[[RFC9110](https://www.rfc-editor.org/rfc/rfc9449.html#RFC9110)\] for credentials and is repeated below for ease of reference. The ABNF notation syntax for DPoP authentication scheme credentials is as follows:
...
[*Figure 12*](https://www.rfc-editor.org/rfc/rfc9449.html#figure-12)*: [DPoP Authentication Scheme ABNF](https://www.rfc-editor.org/rfc/rfc9449.html#name-dpop-authentication-scheme-)*  
For such an access token, a resource server **MUST** check that a DPoP proof was also received in the `DPoP` header field of the HTTP request, check the DPoP proof according to the rules in [Section 4.3](https://www.rfc-editor.org/rfc/rfc9449.html#checking), and check that the public key of the DPoP proof matches the public key to which the access token is bound per [Section 6](https://www.rfc-editor.org/rfc/rfc9449.html#Confirmation).

The resource server **MUST NOT** grant access to the resource unless all checks are successful.

[Figure 13](https://www.rfc-editor.org/rfc/rfc9449.html#protected-resource-request) shows an example request to a protected resource with a DPoP-bound access token in the `Authorization` header and the DPoP proof in the `DPoP` header. The example uses "\\" line wrapping per \[[RFC8792](https://www.rfc-editor.org/rfc/rfc9449.html#RFC8792)\]. [Figure 14](https://www.rfc-editor.org/rfc/rfc9449.html#dpop-proof-pr) shows the decoded content of that DPoP proof. The JSON of the JWT header and payload are shown, but the signature part is omitted. As usual, line breaks and indentation are included for formatting and readability.
...
[*Figure 13*](https://www.rfc-editor.org/rfc/rfc9449.html#figure-13)*: [DPoP-Protected Resource Request](https://www.rfc-editor.org/rfc/rfc9449.html#name-dpop-protected-resource-req)*
...
[*Figure 14*](https://www.rfc-editor.org/rfc/rfc9449.html#figure-14)*: [Decoded Content of the `DPoP` Proof JWT in Figure 13](https://www.rfc-editor.org/rfc/rfc9449.html#name-decoded-content-of-the-dpop)*  
Upon receipt of a request to a protected resource within the protection space requiring DPoP authentication, the server can respond with a challenge to the client to provide DPoP authentication information if the request does not include valid credentials or does not contain an access token sufficient for access. Such a challenge is made using the 401 (Unauthorized) response status code (\[[RFC9110](https://www.rfc-editor.org/rfc/rfc9449.html#RFC9110)\], [Section 15.5.2](https://rfc-editor.org/rfc/rfc9110#section-15.5.2)) and the `WWW-Authenticate` header field (\[[RFC9110](https://www.rfc-editor.org/rfc/rfc9449.html#RFC9110)\], [Section 11.6.1](https://rfc-editor.org/rfc/rfc9110#section-11.6.1)). The server **MAY** include the `WWW-Authenticate` header in response to other conditions as well.

In such challenges:

* The scheme name is `DPoP`.  
* The authentication parameter `realm` **MAY** be included to indicate the scope of protection in the manner described in \[[RFC9110](https://www.rfc-editor.org/rfc/rfc9449.html#RFC9110)\], [Section 11.5](https://rfc-editor.org/rfc/rfc9110#section-11.5).  
* A `scope` authentication parameter **MAY** be included as defined in \[[RFC6750](https://www.rfc-editor.org/rfc/rfc9449.html#RFC6750)\], [Section 3](https://rfc-editor.org/rfc/rfc6750#section-3).  
* An `error` parameter (\[[RFC6750](https://www.rfc-editor.org/rfc/rfc9449.html#RFC6750)\], [Section 3](https://rfc-editor.org/rfc/rfc6750#section-3)) **SHOULD** be included to indicate the reason why the request was declined, if the request included an access token but failed authentication. The error parameter values described in \[[RFC6750](https://www.rfc-editor.org/rfc/rfc9449.html#RFC6750)\], [Section 3.1](https://rfc-editor.org/rfc/rfc6750#section-3.1) are suitable, as are any appropriate values defined by extension. The value `use_dpop_nonce` can be used as described in [Section 9](https://www.rfc-editor.org/rfc/rfc9449.html#RSNonce) to signal that a nonce is needed in the DPoP proof of a subsequent request(s). Additionally, `invalid_dpop_proof` is used to indicate that the DPoP proof itself was deemed invalid based on the criteria of [Section 4.3](https://www.rfc-editor.org/rfc/rfc9449.html#checking).  
* An `error_description` parameter (\[[RFC6750](https://www.rfc-editor.org/rfc/rfc9449.html#RFC6750)\], [Section 3](https://rfc-editor.org/rfc/rfc6750#section-3)) **MAY** be included along with the `error` parameter to provide developers a human-readable explanation that is not meant to be displayed to end-users.  
* An `algs` parameter **SHOULD** be included to signal to the client the JWS algorithms that are acceptable for the DPoP proof JWT. The value of the parameter is a space-delimited list of JWS `alg` (Algorithm) header values (\[[RFC7515](https://www.rfc-editor.org/rfc/rfc9449.html#RFC7515)\], [Section 4.1.1](https://rfc-editor.org/rfc/rfc7515#section-4.1.1)).  
* Additional authentication parameters **MAY** be used, and unknown parameters **MUST** be ignored by recipients.

[Figure 15](https://www.rfc-editor.org/rfc/rfc9449.html#http-401-response-protected-req-without-auth) shows a response to a protected resource request without authentication.

`HTTP/1.1 401 Unauthorized`

 `WWW-Authenticate: DPoP algs="ES256 PS256"`

[*Figure 15*](https://www.rfc-editor.org/rfc/rfc9449.html#figure-15)*: [HTTP 401 Response to a Protected Resource Request without Authentication](https://www.rfc-editor.org/rfc/rfc9449.html#name-http-401-response-to-a-prot)*  
[Figure 16](https://www.rfc-editor.org/rfc/rfc9449.html#http-401-response-protected-req-with-invalid-token) shows a response to a protected resource request that was rejected due to the failed confirmation of the DPoP binding in the access token. [Figure 16](https://www.rfc-editor.org/rfc/rfc9449.html#http-401-response-protected-req-with-invalid-token) uses "\\" line wrapping per \[[RFC8792](https://www.rfc-editor.org/rfc/rfc9449.html#RFC8792)\].
...
Note that the syntax of the `Authorization` header field for this authentication scheme follows the usage of the `Bearer` scheme defined in [Section 2.1](https://rfc-editor.org/rfc/rfc6750#section-2.1) of \[[RFC6750](https://www.rfc-editor.org/rfc/rfc9449.html#RFC6750)\]. While it is not the preferred credential syntax of \[[RFC9110](https://www.rfc-editor.org/rfc/rfc9449.html#RFC9110)\], it is compatible with the general authentication framework therein and is used for consistency and familiarity with the `Bearer` scheme.

### [**7.2.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-7.2) **[Compatibility with the Bearer Authentication Scheme](https://www.rfc-editor.org/rfc/rfc9449.html#name-compatibility-with-the-bear)**

Protected resources simultaneously supporting both the `DPoP` and `Bearer` schemes need to update how the evaluation process is performed for bearer tokens to prevent downgraded usage of a DPoP-bound access token. Specifically, such a protected resource **MUST** reject a DPoP-bound access token received as a bearer token per \[[RFC6750](https://www.rfc-editor.org/rfc/rfc9449.html#RFC6750)\].

[Section 11.6.1](https://rfc-editor.org/rfc/rfc9110#section-11.6.1) of \[[RFC9110](https://www.rfc-editor.org/rfc/rfc9449.html#RFC9110)\] allows a protected resource to indicate support for multiple authentication schemes (i.e., `Bearer` and `DPoP`) with the `WWW-Authenticate` header field of a 401 (Unauthorized) response.

A protected resource that supports only \[[RFC6750](https://www.rfc-editor.org/rfc/rfc9449.html#RFC6750)\] and is unaware of DPoP would most presumably accept a DPoP-bound access token as a bearer token (JWT \[[RFC7519](https://www.rfc-editor.org/rfc/rfc9449.html#RFC7519)\] says to ignore unrecognized claims, Introspection \[[RFC7662](https://www.rfc-editor.org/rfc/rfc9449.html#RFC7662)\] says that other parameters might be present while placing no functional requirements on their presence, and \[[RFC6750](https://www.rfc-editor.org/rfc/rfc9449.html#RFC6750)\] is effectively silent on the content of the access token since it relates to validity). As such, a client can send a DPoP-bound access token using the `Bearer` scheme upon receipt of a `WWW-Authenticate: Bearer` challenge from a protected resource (or it can send a DPoP-bound access token if it has prior knowledge of the capabilities of the protected resource). The effect of this likely simplifies the logistics of phased upgrades to protected resources in their support DPoP or prolonged deployments of protected resources with mixed token type support.
...
* If no authentication information has been included with the request, then the challenges **SHOULD NOT** include an error code or other error information, as per [Section 3.1](https://rfc-editor.org/rfc/rfc6750#section-3.1) of \[[RFC6750](https://www.rfc-editor.org/rfc/rfc9449.html#RFC6750)\] ([Figure 17](https://www.rfc-editor.org/rfc/rfc9449.html#multi-challenge-no-token)).  
* If the mechanism used to attempt authentication could be established unambiguously, then the corresponding challenge **SHOULD** be used to deliver error information ([Figure 18](https://www.rfc-editor.org/rfc/rfc9449.html#multi-challenge-invalid-token)).  
* Otherwise, both `Bearer` and `DPoP` challenges **MAY** be used to deliver error information ([Figure 19](https://www.rfc-editor.org/rfc/rfc9449.html#multi-challenge-ambiguous)).

The following examples use "\\" line wrapping per \[[RFC8792](https://www.rfc-editor.org/rfc/rfc9449.html#RFC8792)\].
...
[*Figure 17*](https://www.rfc-editor.org/rfc/rfc9449.html#figure-17)*: [HTTP 401 Response to a Protected Resource Request without Authentication](https://www.rfc-editor.org/rfc/rfc9449.html#name-http-401-response-to-a-protec)*
...
[*Figure 18*](https://www.rfc-editor.org/rfc/rfc9449.html#figure-18)*: [HTTP 401 Response to a Protected Resource Request with Invalid Authentication](https://www.rfc-editor.org/rfc/rfc9449.html#name-http-401-response-to-a-protect)*
...
[*Figure 19*](https://www.rfc-editor.org/rfc/rfc9449.html#figure-19)*: [HTTP 400 Response to a Protected Resource Request with Ambiguous Authentication](https://www.rfc-editor.org/rfc/rfc9449.html#name-http-400-response-to-a-prot)*

### [**7.3.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-7.3) **[Client Considerations](https://www.rfc-editor.org/rfc/rfc9449.html#name-client-considerations)**
...
## [**8\.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-8) **[Authorization Server-Provided Nonce](https://www.rfc-editor.org/rfc/rfc9449.html#name-authorization-server-provid)**
...
[*Figure 20*](https://www.rfc-editor.org/rfc/rfc9449.html#figure-20)*: [HTTP 400 Response to a Token Request without a Nonce](https://www.rfc-editor.org/rfc/rfc9449.html#name-http-400-response-to-a-toke)*  
...
[*Figure 21*](https://www.rfc-editor.org/rfc/rfc9449.html#figure-21)*: [DPoP Proof Payload including a Nonce Value](https://www.rfc-editor.org/rfc/rfc9449.html#name-dpop-proof-payload-includin)*  
...
### [**8.1.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-8.1) **[Nonce Syntax](https://www.rfc-editor.org/rfc/rfc9449.html#name-nonce-syntax)**

The nonce syntax in ABNF as used by \[[RFC6749](https://www.rfc-editor.org/rfc/rfc9449.html#RFC6749)\] (which is the same as the scope-token syntax) is shown below.

`nonce = 1*NQCHAR`  
[*Figure 22*](https://www.rfc-editor.org/rfc/rfc9449.html#figure-22)*: [Nonce ABNF](https://www.rfc-editor.org/rfc/rfc9449.html#name-nonce-abnf)*

### [**8.2.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-8.2) **[Providing a New Nonce Value](https://www.rfc-editor.org/rfc/rfc9449.html#name-providing-a-new-nonce-value)**
...
The authorization server **MAY** supply the new nonce in the same way that the initial one was supplied: by using a `DPoP-Nonce` HTTP header in the response. The `DPoP-Nonce` HTTP header field uses the nonce syntax defined in [Section 8.1](https://www.rfc-editor.org/rfc/rfc9449.html#NonceSyntax). Each time this happens, it requires an extra protocol round trip.
...
[*Figure 23*](https://www.rfc-editor.org/rfc/rfc9449.html#figure-23)*: [HTTP 200 Response Providing the Next Nonce Value](https://www.rfc-editor.org/rfc/rfc9449.html#name-http-200-response-providing)*

## [**9\.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-9) **[Resource Server-Provided Nonce](https://www.rfc-editor.org/rfc/rfc9449.html#name-resource-server-provided-no)**

Resource servers can also choose to provide a nonce value to be included in DPoP proofs sent to them. They provide the nonce using the `DPoP-Nonce` header in the same way that authorization servers do as described in Sections [8](https://www.rfc-editor.org/rfc/rfc9449.html#ASNonce) and [8.2](https://www.rfc-editor.org/rfc/rfc9449.html#NewNonce). The error signaling is performed as described in [Section 7.1](https://www.rfc-editor.org/rfc/rfc9449.html#http-auth-scheme). Resource servers use an HTTP 401 (Unauthorized) error code with an accompanying `WWW-Authenticate: DPoP` value and `DPoP-Nonce` value to accomplish this.

For example, in response to a resource request without a nonce when the resource server requires one, the resource server can respond with a `DPoP-Nonce` value such as the following to provide a nonce value to include in the DPoP proof. The example below uses "\\" line wrapping per \[[RFC8792](https://www.rfc-editor.org/rfc/rfc9449.html#RFC8792)\].
...
An example authorization request using the `dpop_jkt` authorization request parameter is shown below and uses "\\" line wrapping per \[[RFC8792](https://www.rfc-editor.org/rfc/rfc9449.html#RFC8792)\].
...
[*Figure 25*](https://www.rfc-editor.org/rfc/rfc9449.html#figure-25)*: [Authorization Request Using the `dpop_jkt` Parameter](https://www.rfc-editor.org/rfc/rfc9449.html#name-authorization-request-using)*  
Use of the `dpop_jkt` authorization request parameter is **OPTIONAL**. Note that the `dpop_jkt` authorization request parameter **MAY** also be used in combination with Proof Key for Code Exchange (PKCE) \[[RFC7636](https://www.rfc-editor.org/rfc/rfc9449.html#RFC7636)\], which is recommended by \[[SECURITY-TOPICS](https://www.rfc-editor.org/rfc/rfc9449.html#I-D.ietf-oauth-security-topics)\] as a countermeasure to authorization code injection. The `dpop_jkt` authorization request parameter only provides similar protections when a unique DPoP key is used for each authorization request.

### [**10.1.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-10.1) **[DPoP with Pushed Authorization Requests](https://www.rfc-editor.org/rfc/rfc9449.html#name-dpop-with-pushed-authorizat)**

When Pushed Authorization Requests (PARs) \[[RFC9126](https://www.rfc-editor.org/rfc/rfc9449.html#RFC9126)\] are used in conjunction with DPoP, there are two ways in which the DPoP key can be communicated in the PAR request:

* The `dpop_jkt` parameter can be used as described in [Section 10](https://www.rfc-editor.org/rfc/rfc9449.html#dpop_jkt) to bind the issued authorization code to a specific key. In this case, `dpop_jkt` **MUST** be included alongside other authorization request parameters in the POST body of the PAR request.  
* Alternatively, the `DPoP` header can be added to the PAR request. In this case, the authorization server **MUST** check the provided DPoP proof JWT as defined in [Section 4.3](https://www.rfc-editor.org/rfc/rfc9449.html#checking). It **MUST** further behave as if the contained public key's thumbprint was provided using `dpop_jkt`, i.e., reject the subsequent token request unless a DPoP proof for the same key is provided. This can help to simplify the implementation of the client, as it can "blindly" attach the `DPoP` header to all requests to the authorization server regardless of the type of request. Additionally, it provides a stronger binding, as the `DPoP` header contains a proof of possession of the private key.
...
## [**11\.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-11) **[Security Considerations](https://www.rfc-editor.org/rfc/rfc9449.html#name-security-considerations)**

In DPoP, the prevention of token replay at a different endpoint (see [Section 2](https://www.rfc-editor.org/rfc/rfc9449.html#objective)) is achieved through authentication of the server per \[[RFC6125](https://www.rfc-editor.org/rfc/rfc9449.html#RFC6125)\] and the binding of the DPoP proof to a certain URI and HTTP method. However, DPoP has a somewhat different nature of protection than TLS-based methods such as OAuth Mutual TLS \[[RFC8705](https://www.rfc-editor.org/rfc/rfc9449.html#RFC8705)\] or OAuth Token Binding \[[TOKEN-BINDING](https://www.rfc-editor.org/rfc/rfc9449.html#I-D.ietf-oauth-token-binding)\] (see also Sections [11.1](https://www.rfc-editor.org/rfc/rfc9449.html#Token_Replay) and [11.7](https://www.rfc-editor.org/rfc/rfc9449.html#request_integrity)). TLS-based mechanisms can leverage a tight integration between the TLS layer and the application layer to achieve strong message integrity, authenticity, and replay protection.

### [**11.1.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-11.1) **[DPoP Proof Replay](https://www.rfc-editor.org/rfc/rfc9449.html#name-dpop-proof-replay)**
...
### [**11.2.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-11.2) **[DPoP Proof Pre-generation](https://www.rfc-editor.org/rfc/rfc9449.html#name-dpop-proof-pre-generation)**
...
### [**11.3.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-11.3) **[DPoP Nonce Downgrade](https://www.rfc-editor.org/rfc/rfc9449.html#name-dpop-nonce-downgrade)**

A server **MUST NOT** accept any DPoP proofs without the `nonce` claim when a DPoP nonce has been provided to the client.

### [**11.4.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-11.4) **[Untrusted Code in the Client Context](https://www.rfc-editor.org/rfc/rfc9449.html#name-untrusted-code-in-the-clien)**
...
Therefore, protecting clients against the execution of untrusted code is extremely important even if DPoP is used. Besides secure coding practices, Content Security Policy \[[W3C.CSP](https://www.rfc-editor.org/rfc/rfc9449.html#W3C.CSP)\] can be used as a second layer of defense against XSS.

### [**11.5.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-11.5) **[Signed JWT Swapping](https://www.rfc-editor.org/rfc/rfc9449.html#name-signed-jwt-swapping)**
...
### [**11.6.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-11.6) **[Signature Algorithms](https://www.rfc-editor.org/rfc/rfc9449.html#name-signature-algorithms)**
...
### [**11.7.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-11.7) **[Request Integrity](https://www.rfc-editor.org/rfc/rfc9449.html#name-request-integrity)**
...
### [**11.8.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-11.8) **[Access Token and Public Key Binding](https://www.rfc-editor.org/rfc/rfc9449.html#name-access-token-and-public-key)**

The binding of the access token to the DPoP public key, as specified in [Section 6](https://www.rfc-editor.org/rfc/rfc9449.html#Confirmation), uses a cryptographic hash of the JWK representation of the public key. It relies on the hash function having sufficient second-preimage resistance so as to make it computationally infeasible to find or create another key that produces to the same hash output value. The SHA-256 hash function was used because it meets the aforementioned requirement while being widely available.

Similarly, the binding of the DPoP proof to the access token uses a hash of that access token as the value of the `ath` claim in the DPoP proof (see [Section 4.2](https://www.rfc-editor.org/rfc/rfc9449.html#DPoP-Proof-Syntax)). This relies on the value of the hash being sufficiently unique so as to reliably identify the access token. The collision resistance of SHA-256 meets that requirement.

### [**11.9.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-11.9) **[Authorization Code and Public Key Binding](https://www.rfc-editor.org/rfc/rfc9449.html#name-authorization-code-and-publ)**

Cryptographic binding of the authorization code to the DPoP public key is specified in [Section 10](https://www.rfc-editor.org/rfc/rfc9449.html#dpop_jkt). This binding prevents attacks in which the attacker captures the authorization code and creates a DPoP proof using a proof-of-possession key other than the one held by the client and redeems the authorization code using that DPoP proof. By ensuring end to end that only the client's DPoP key can be used, this prevents captured authorization codes from being exfiltrated and used at locations other than the one to which the authorization code was issued.
...
### [**11.10.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-11.10) **[Hash Algorithm Agility](https://www.rfc-editor.org/rfc/rfc9449.html#name-hash-algorithm-agility)**
...
### [**11.11.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-11.11) **[Binding to Client Identity](https://www.rfc-editor.org/rfc/rfc9449.html#name-binding-to-client-identity)**
...
## [**12\.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-12) **[IANA Considerations](https://www.rfc-editor.org/rfc/rfc9449.html#name-iana-considerations)**

### [**12.1.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-12.1) **[OAuth Access Token Types Registration](https://www.rfc-editor.org/rfc/rfc9449.html#name-oauth-access-token-types-re)**

IANA has registered the following access token type in the "OAuth Access Token Types" registry \[[IANA.OAuth.Params](https://www.rfc-editor.org/rfc/rfc9449.html#IANA.OAuth.Params)\] established by \[[RFC6749](https://www.rfc-editor.org/rfc/rfc9449.html#RFC6749)\].
...
### [**12.2.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-12.2) **[OAuth Extensions Error Registration](https://www.rfc-editor.org/rfc/rfc9449.html#name-oauth-extensions-error-regi)**

IANA has registered the following error values in the "OAuth Extensions Error" registry \[[IANA.OAuth.Params](https://www.rfc-editor.org/rfc/rfc9449.html#IANA.OAuth.Params)\] established by \[[RFC6749](https://www.rfc-editor.org/rfc/rfc9449.html#RFC6749)\].
...
### [**12.3.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-12.3) **[OAuth Parameters Registration](https://www.rfc-editor.org/rfc/rfc9449.html#name-oauth-parameters-registrati)**

IANA has registered the following authorization request parameter in the "OAuth Parameters" registry \[[IANA.OAuth.Params](https://www.rfc-editor.org/rfc/rfc9449.html#IANA.OAuth.Params)\] established by \[[RFC6749](https://www.rfc-editor.org/rfc/rfc9449.html#RFC6749)\].
...
Reference:  
[Section 10](https://www.rfc-editor.org/rfc/rfc9449.html#dpop_jkt) of RFC 9449

### [**12.4.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-12.4) **[HTTP Authentication Schemes Registration](https://www.rfc-editor.org/rfc/rfc9449.html#name-http-authentication-schemes)**

IANA has registered the following scheme in the "HTTP Authentication Schemes" registry \[[IANA.HTTP.AuthSchemes](https://www.rfc-editor.org/rfc/rfc9449.html#IANA.HTTP.AuthSchemes)\] established by \[[RFC9110](https://www.rfc-editor.org/rfc/rfc9449.html#RFC9110)\], [Section 16.4.1](https://rfc-editor.org/rfc/rfc9110#section-16.4.1).

Authentication Scheme Name:  
`DPoP`

Reference:  
[Section 7.1](https://www.rfc-editor.org/rfc/rfc9449.html#http-auth-scheme) of RFC 9449

### [**12.5.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-12.5) **[Media Type Registration](https://www.rfc-editor.org/rfc/rfc9449.html#name-media-type-registration)**

IANA has registered the `application/dpop+jwt` media type \[[RFC2046](https://www.rfc-editor.org/rfc/rfc9449.html#RFC2046)\] in the "Media Types" registry \[[IANA.MediaTypes](https://www.rfc-editor.org/rfc/rfc9449.html#IANA.MediaTypes)\] in the manner described in \[[RFC6838](https://www.rfc-editor.org/rfc/rfc9449.html#RFC6838)\], which is used to indicate that the content is a DPoP JWT.
...
See [Section 11](https://www.rfc-editor.org/rfc/rfc9449.html#Security) of RFC 9449
...
Michael B. Jones, michael\_b\_jones@hotmail.com
...
### [**12.6.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-12.6) **[JWT Confirmation Methods Registration](https://www.rfc-editor.org/rfc/rfc9449.html#name-jwt-confirmation-methods-re)**

IANA has registered the following JWT `cnf` member value in the "JWT Confirmation Methods" registry \[[IANA.JWT](https://www.rfc-editor.org/rfc/rfc9449.html#IANA.JWT)\] established by \[[RFC7800](https://www.rfc-editor.org/rfc/rfc9449.html#RFC7800)\].
...
Reference:  
[Section 6](https://www.rfc-editor.org/rfc/rfc9449.html#Confirmation) of RFC 9449

### [**12.7.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-12.7) **[JSON Web Token Claims Registration](https://www.rfc-editor.org/rfc/rfc9449.html#name-json-web-token-claims-regis)**

IANA has registered the following Claims in the "JSON Web Token Claims" registry \[[IANA.JWT](https://www.rfc-editor.org/rfc/rfc9449.html#IANA.JWT)\] established by \[[RFC7519](https://www.rfc-editor.org/rfc/rfc9449.html#RFC7519)\].
...
[Section 4.2](https://www.rfc-editor.org/rfc/rfc9449.html#DPoP-Proof-Syntax) of RFC 9449
...
#### [**12.7.1.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-12.7.1) **["nonce" Registration Update](https://www.rfc-editor.org/rfc/rfc9449.html#name-nonce-registration-update)**
...
However, the initial registration of the `nonce` claim by \[[OpenID.Core](https://www.rfc-editor.org/rfc/rfc9449.html#OpenID.Core)\] used language that was contextually specific to that application, which was potentially limiting to its general applicability.

Therefore, IANA has updated the entry for `nonce` in the "JSON Web Token Claims" registry \[[IANA.JWT](https://www.rfc-editor.org/rfc/rfc9449.html#IANA.JWT)\] with an expanded definition to reflect that the claim can be used appropriately in other contexts and with the addition of this document as a reference, as follows.
...
[Section 2](https://openid.net/specs/openid-connect-core-1_0.html#IDToken) of \[[OpenID.Core](https://www.rfc-editor.org/rfc/rfc9449.html#OpenID.Core)\] and RFC 9449

### [**12.8.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-12.8) **[Hypertext Transfer Protocol (HTTP) Field Name Registration](https://www.rfc-editor.org/rfc/rfc9449.html#name-hypertext-transfer-protocol)**

IANA has registered the following HTTP header fields, as specified by this document, in the "Hypertext Transfer Protocol (HTTP) Field Name Registry" \[[IANA.HTTP.Fields](https://www.rfc-editor.org/rfc/rfc9449.html#IANA.HTTP.Fields)\] established by \[[RFC9110](https://www.rfc-editor.org/rfc/rfc9449.html#RFC9110)\]:
...
### [**12.9.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-12.9) **[OAuth Authorization Server Metadata Registration](https://www.rfc-editor.org/rfc/rfc9449.html#name-oauth-authorization-server-)**

IANA has registered the following value in the "OAuth Authorization Server Metadata" registry \[[IANA.OAuth.Params](https://www.rfc-editor.org/rfc/rfc9449.html#IANA.OAuth.Params)\] established by \[[RFC8414](https://www.rfc-editor.org/rfc/rfc9449.html#RFC8414)\].
...
Reference:  
[Section 5.1](https://www.rfc-editor.org/rfc/rfc9449.html#as-meta) of RFC 9449

### [**12.10.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-12.10) **[OAuth Dynamic Client Registration Metadata](https://www.rfc-editor.org/rfc/rfc9449.html#name-oauth-dynamic-client-regist)**

IANA has registered the following value in the IANA "OAuth Dynamic Client Registration Metadata" registry \[[IANA.OAuth.Params](https://www.rfc-editor.org/rfc/rfc9449.html#IANA.OAuth.Params)\] established by \[[RFC7591](https://www.rfc-editor.org/rfc/rfc9449.html#RFC7591)\].
...
Reference:  
[Section 5.2](https://www.rfc-editor.org/rfc/rfc9449.html#client-meta) of RFC 9449

## [**13\.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-13) **[References](https://www.rfc-editor.org/rfc/rfc9449.html#name-references)**

### [**13.1.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-13.1) **[Normative References](https://www.rfc-editor.org/rfc/rfc9449.html#name-normative-references)**
...
### [**13.2.**](https://www.rfc-editor.org/rfc/rfc9449.html#section-13.2) **[Informative References](https://www.rfc-editor.org/rfc/rfc9449.html#name-informative-references)**
...
## [**Acknowledgements**](https://www.rfc-editor.org/rfc/rfc9449.html#name-acknowledgements)
...
## [**Authors' Addresses**](https://www.rfc-editor.org/rfc/rfc9449.html#name-authors-addresses)
